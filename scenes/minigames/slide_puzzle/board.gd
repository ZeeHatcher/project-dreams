...
func create_board():
	# tile pos an size
	var current_x: float = grid_position.x + tile_margin
	var current_y: float = grid_position.y + tile_margin
	...
	for i in range(grid_size * grid_size):
		...
		# get next position
		if (i + 1) % grid_size > 0:
			current_x += width + (2 * tile_margin)
			
		else:
			# jump line
			current_y += height + (2 * tile_margin)
			current_x = grid_position.x + tile_margin
